...
This is the modification of test code that evaluates different perturbation sigmas on validation set
...
# File that will store losses for each sigma
f = open(output_file, "w")

# Placeholders for best val loss
best_mae, best_rmse, best_relatives, best_sigma = None, None, None, None

# Check all even sigmas between 2 and 20
for sigma in 2 * np.arange(1, 11):
    ...
    for im_id in pbar:
        ...
        for step in range(0, args.gradient_steps):
            ...
            lPerturbation = args.weight_perturbation * PerturbationLoss(output, boxes, sigma=sigma)
    ...
    # Check, whether this sigma improved the results
    mae = np.mean(SAE)
    if best_mae is None or mae < best_mae:
        best_mae = mae
        best_rmse = np.sqrt(np.mean(SSE))
        best_relatives = relative_errors
        best_sigma = sigma

    # Store the results for current sigma
...
print('On {} data, MAE: {:6.2f}, RMSE: {:6.2f}'.format(args.test_split, best_mae, best_rmse))

relative_errors = {k: v for k, v in sorted(best_relatives.items(), key=lambda item: item[1])}
