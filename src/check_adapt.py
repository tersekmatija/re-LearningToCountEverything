...
cnt_adapt = 0
SAE_adapt = []  # sum of absolute errors
SSE_adapt = [] # sum of square errors

relative_errors_adapt = dict()
...
for im_id in pbar:
    ...
    gt_cnt = dots.shape[0]
    pred_cnt = output.sum().item()
    cnt_adapt = cnt_adapt + 1
    err = abs(gt_cnt - pred_cnt)
    SAE_adapt.append(err)
    SSE_adapt.append(err**2)

    pbar.set_description('{:<8}: actual-predicted: {:6d}, {:6.1f}, error: {:6.1f}. Current MAE: {:5.2f}, RMSE: {:5.2f}'.\
                         format(im_id, gt_cnt, pred_cnt, abs(pred_cnt - gt_cnt), np.mean(SAE_adapt), np.sqrt(np.mean(SSE_adapt))))
    print("")
    relative_errors_adapt[im_id] = abs(pred_cnt - gt_cnt) / gt_cnt
    

relative_errors_diff = {k: relative_errors[k] - relative_errors_adapt[k] for k in relative_errors}
relative_errors_diff = {k: v for k, v in sorted(relative_errors_diff.items(), key=lambda item: -item[1])}
best_improvement = list(relative_errors_diff.items())[:3]
worst_improvement = list(relative_errors_diff.items())[-3:]

with open(output_file, "w") as f:
    f.write("3 images with the biggest improvement in relative error (image: change of RE):\n")
    for image, change in best_improvement:
        f.write(f"{image}: {change}\n")
    
    f.write("3 images with the worst improvement in relative error:\n")
    for image, change in worst_improvement:
        f.write(f"{image}: {change}\n")