...
with torch.no_grad():
    ...
    for i in range(rows):
        for j in range(columns):
            ...
            ax.set_axis_off()
            ax.imshow(features[0,i,j,:,:])
           
            print(f"SUM: {features[0,i,j,:,:].sum()}")
            c += 1
    plt.tight_layout()
    features_file = "{}/{}_features.png".format(args.output_dir, image_name)
    fig.savefig(features_file, bbox_inches = "tight")
